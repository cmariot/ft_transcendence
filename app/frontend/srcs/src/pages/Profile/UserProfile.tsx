...
const UserProfile = () => {
  ...
  function friendOrUnfriend(username: string) {
    ...
    if (already_friend === true) {
      return (
        <button onClick={() => removeFriend(username)}>remove friend</button>
      );
    } else {
      return <button onClick={() => addFriend(username)}>add friend</button>;
    }
  }
    ...
    if (found) {
      return (
        <>
          ...
          <img id="profile-user-picture" src={userImage} alt="user-imag" />
          <h3>{username}</h3>
          <div>
