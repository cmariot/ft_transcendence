...
const YourChannels = () => {
  ...
  async function joinChannel(channel: string, channelType: string) {
    await axios
      ...
      .then(function (response) {
        ...
        chat.setCurrentChannelUsers(response.data.private_channel_users);
        ...
        <>
          {Array.from(chat.userPrivateChannels).map((item, index) => (
            <button
              ...
              key={index}
              onClick={() => joinChannel(item[0], item[1].channelType)}
            >
              <p className="channel-selection-button-channel-name">{item[0]}</p>
            </button>
          ))}
        </>
      );
  ...
  function nothinToDisplay() {
    return chat.userChannels.size === 0 && chat.userPrivateChannels.size === 0;
  }
  ...
  return (
    <menu id="chat-your-channels" className="chat-menu">
      ...
      <footer className="chat-footer">
        <button onClick={() => viewChannelsList()}>browse channels</button>
      </footer>
    </menu>
  );
};
