        ...
        if (response.status === HttpStatusCode.NoContent) {
          ...
          axios
            ...
            .then(function (response2) {
              ...
              chat.changeCurrentChannelType(response.data.channelType);
              chat.setCurrentChannelMessages(response2.data.messages);
              chat.setChannelOwner(response2.data.channel_owner);
              chat.setChannelAdmin(response2.data.channel_admin);
              chat.setCurrentChannelAdmins(response2.data.channel_admins);
              chat.setCurrentChannelMute(response2.data.muted_users);
              chat.setCurrentChannelBan(response2.data.banned_users);
              chat.setCurrentChannelUsers(response2.data.private_channel_users);
            })
  ...
  function messageMenu(username: string) {
    if (username !== user.username) {
      if (chat.currentChannelType === "private") {
        ...
        return (
          <div className="chat-user-message-options">
            <button onClick={() => directMessage(username)}>message</button>
            <button>play</button>
            <button onClick={() => profile(username)}>profile</button>
            <button onClick={() => blockUser(username)}>block</button>
          </div>
