...
export const ChatParent = () => {
  ...
  useEffect(() => {
    ...
    socket.on("banUser", (socket) => {
      if (
        ...
        if (currentAvailableChannels.has(socket.channelName) === false) {
    ...
    socket.on("newChatMessage", (socket) => {
      ...
      if (socketChannel === currentChannel) {
        axios
          ...
          .then((response) => setCurrentChannelMessages(response.data.messages))
          .catch((error) => console.log(error.data));
      }
    });
  }, [currentChannel]);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        let initialUserChannels = new Map<string, { channelType: string }>();
        ...
        let initialUserPrivateChannels = new Map<
          ...
          { channelType: string }
        >();
        for (let i = 0; i < response.data.userPrivateChannels.length; i++) {
          if (
        ...
        let initialAvailableChannels = new Map<
          ...
          { channelType: string }
        >();
        for (let i = 0; i < response.data.availableChannels.length; i++) {
          if (
