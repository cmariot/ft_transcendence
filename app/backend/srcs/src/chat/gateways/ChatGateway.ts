...
export class ChatGateway implements OnModuleInit {
  ...
  onModuleInit() {
    this.server.on("connection", (socket) => {
      socket.on("disconnect", async () => {
        let username = await this.userService.userDisconnection(socket.id);
        if (username) {
    ...
    while (i < user.socketId.length) {
      ...
      if (socket) {
        for (let room of socket.rooms) {
          let valid = true;
          for (let k = 0; k < "chatroom_".length && k < room[k].length; k++) {
  ...
  async userStatus(@MessageBody() data: any) {
    ...
    if (username && socketID && status === "Online") {
      user = await this.userService.setSocketID(username, socketID, status);
    }
