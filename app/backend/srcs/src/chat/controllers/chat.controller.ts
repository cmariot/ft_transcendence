...
export class ChatController {
  ...
  @Post("create/private")
  @UseGuards(isLogged)
  async create_private_conv(@Req() req, @Body() newChannel: PrivateMessageDTO) {
  ...
  channel_connection(@Req() req, @Body() channel: channelDTO) {
    return this.chatService.join_channel(channel.channelName, req.user.uuid);
  }

  @Post("leave")
  @UseGuards(isLogged)
  leave_channel(@Req() req, @Body() channel: channelDTO) {
    return this.chatService.leave_channel(channel.channelName, req.user.uuid);
  }
  ...
  async updateChannelPassword(@Req() req, @Body() channel: updateChannelDTO) {
    ...
    let targetChannel = await this.chatService.getByName(channel.channelName);
  ...
  async add_admin(@Req() req, @Body() channel: addAdminDTO) {
    ...
    return this.chatService.addAdmin(targetChannel, newAdmin.uuid, user.uuid);
  }
  ...
  async ban_user(@Req() req, @Body() muteOptions: muteOptionsDTO) {
    ...
    let mutedUser = await this.userService.getByUsername(muteOptions.username);
    if (!mutedUser) {
      throw new HttpException("User not found", HttpStatus.FOUND);
    }
    ...
    return this.chatService.ban(user, mutedUser, targetChannel, muteOptions);
  }
  ...
  async kick_user(@Req() req, @Body() options: kickOptionsDTO) {
    ...
    let targetChannel = await this.chatService.getByName(options.channelName);
    if (!targetChannel) {
      throw new HttpException("Invalid channel", HttpStatus.FOUND);
    }
    if (
  ...
  async mute_user(@Req() req, @Body() muteOptions: muteOptionsDTO) {
    ...
    return this.chatService.mute(user, mutedUser, targetChannel, muteOptions);
  }
  ...
  async get_banned_channel_users(@Req() req, @Body() channel: channelDTO) {
    return await this.chatService.banList(channel.channelName, req.user.uuid);
  }
}
