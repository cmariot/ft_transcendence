...
export class ChatService {
  ...
  async get_channels(uuid: string) {
    ...
    if (!user) {
      throw new HttpException("Error, invalid user", HttpStatus.BAD_REQUEST);
    }
        ...
        while (j < channels[i].users.length) {
          if (channels[i].users[j].uuid === user.uuid) {
            ...
            while (k < channels[i].banned_users.length) {
              if (channels[i].banned_users[k].uuid === user.uuid) {
                banned = true;
                break;
              }
              k++;
            ...
            if (!blocked_users) {
              ...
              while (k < channels[i].allowed_users.length) {
                ...
                while (l < blocked_users.length) {
                  if (
                    channels[i].allowed_users[k].uuid === blocked_users[l] &&
                    channels[i].channelType === ChannelType.PRIVATE
                  ) {
                    found = true;
                    break;
                    ...
                    channels[i].channelType === ChannelType.PRIVATE_CHANNEL &&
                    blocked_users[l] === channels[i].channelOwner
                  ) {
                    found = true;
                    break;
                  }
  ...
  async create_channel(newChannel: any, uuid: string) {
    ...
    if (channel) {
      throw new HttpException("Unavailable channel name", HttpStatus.FOUND);
    }
    ...
    if (newChannel.channelType === ChannelType.PRIVATE_CHANNEL) {
      ...
      this.chatGateway.userJoinChannel(channel.channelName, user.username);
      return await this.convertChannelMessages(uuid, channel.messages, channel);
    }
  ...
  async join_channel(channelName: string, userID: string) {
    ...
    if (
      ...
      if (index_in_authorized_channels !== -1) {
        ...
        let allowed_user = channel.allowed_users.findIndex((element) => {
      ...
      return this.convertChannelMessages(user.uuid, channel.messages, channel);
      ...
      throw new HttpException("Enter the channel's password", HttpStatus.FOUND);
    }
    throw new UnauthorizedException();
  }
      ...
      if (channels[i].channelType === ChannelType.PRIVATE) {
        ...
        while (j < channels[i].allowed_users.length) {
          if (channels[i].allowed_users[j].uuid === uuid) {
            ...
            while (k < channels[i].allowed_users.length) {
              if (
                channels[i].allowed_users[k].uuid === friend.uuid &&
                channels[i].allowed_users.length === 2
              ) {
                //join ici
                let messages = await this.convertChannelMessages(
                  uuid,
                  channels[i].messages,
                  channels[i]
                );
  ...
  async leave_channel(channelName: string, uuid: string) {
    ...
    if (
      ...
      if (channel.channelOwner === uuid) {
        this.chatGateway.deleted_channel(channel.channelName, user.username);
        ...
        if (index !== -1) {
          ...
          if (index !== -1) {
            ...
            await this.chatRepository.update(
              { channelName: channelName },
              {
                channelAdministrators: channel.channelAdministrators,
              }
            );
          }
        } else {
  ...
  async addAdmin(channel: ChatEntity, newAdminUUID: string, ownerUUID: string) {
  ...
  async ban(
    ...
    if (found === false) {
      ...
      setTimeout(() => {
        this.timeout(user, mutedUser, targetChannel, muteOptions, "ban");
      }, Number(muteOptions.duration) * 1000);
    }
    await this.chatRepository.update(
      { uuid: targetChannel.uuid },
      { banned_users: currentBanned }
    );
    this.chatGateway.ban_user(targetChannel.channelName, muteOptions.username);
    ...
    if (
      ...
      for (let i = 0; i < current_users.length; i++) {
        if (current_users[i].uuid === kickedUser.uuid) {
          index = i;
          this.leave_channel(targetChannel.channelName, current_users[i].uuid);
        ...
        if (current_users[i].uuid === kickedUser.uuid) {
          ...
          while (j < targetChannel.allowed_users.length) {
            if (
              ...
              targetChannel.allowed_users[j].uuid !== kickedUser.uuid
            ) {
              let username = await this.userService.getUsernameById(
